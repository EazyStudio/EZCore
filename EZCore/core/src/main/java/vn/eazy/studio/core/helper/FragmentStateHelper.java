...
public class FragmentStateHelper implements OnFragmentStateAction {
    ...
    private List<Stack<Fragment>> stacksFragment;
    ...
    public List<Stack<Fragment>> getStacksFragment() {
    ...
    @Override
    public void setStacksRootFragment(Fragment... fragments) {
        rootFragments = new Fragment[fragments.length];
        for (int index = 0; index < fragments.length; index++) {
            stacksFragment.add(new Stack<Fragment>());
        }
        rootFragments = fragments;
    }

    @Override
    public void changeRootFragment(Fragment fragments, int stackId) {
        clearStack(stackId);
    ...
    @Override
    public void pushFragmentKeepOld(Fragment fragment) {
        beginTrans().add(idContent, fragment, generateTag(fragment)).commitAllowingStateLoss();
        stacksFragment.get(stackSelected).push(fragment);
    ...
    public void clearStack(int stackId) {
        Stack<Fragment> stackFragment = stacksFragment.get(stackId);
    ...
    public void clearAllStacks() {
        for (Stack<Fragment> stack : stacksFragment) {
